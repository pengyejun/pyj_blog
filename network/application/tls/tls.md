...
- 由上图可知，证书是一个证书链，服务端的证书会在第一级，中间证书证书在最后一级。在验证证书时会从第一级证书开始往下查找，当查找到浏览器包含证书的签发机构时。若查找完后都未发现可信任的证书签发机构，浏览器则会提示此站点不可信任。
  
- 由于百度的证书在我的浏览器中并未内置，因此百度的证书不可信任，接着逐级查找，找到中间证书的签发机构时，在浏览器内置证书中找到了此机构`GlobalSign nv-sa`，证书如下:
...
- 验证百度站点证书
    ...
    验证逻辑与中间证书验证一致，不同的点在于，证书公钥取中间证书中的`subjectPublicKey`，如下所示：

    ![GlobalSign na-sa pubkey](./GlobalSign%20na-sa%20pubkey.png)

    获取了证书公钥之后就可以根据中间证书验证方法进行验证。
